export const movies = [
    {
      id: 1,
      title: 'Redemption',
      year: 1994,
      genre: ['Drama'],
      rating: 9,
      director: 'Frank Darabont',
      writer: 'Stephen King',
      cast: ['Tim Robbins', 'Morgan Freeman'],
      summary:
        'Two imprisoned men bond over several years, finding solace and eventual redemption through acts of common decency.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BNDE3ODcxYzMtY2YzZC00NmNlLWJiNDMtZDViZWM2MzIxZDYwXkEyXkFqcGdeQXVyNjAwNDUxODI@._V1_FMjpg_UX1000_.jpg',
    },
    {
      id: 2,
      title: 'The Godfather',
      year: 1992,
      genre: ['Crime', 'Drama'],
      rating: 9,
      director: 'Francis Ford Coppola',
      writer: 'Mario Puzo',
      cast: ['Marlon Brando', 'Al Pacino'],
      summary:
        'The aging patriarch of an organized crime dynasty transfers control of his clandestine empire to his reluctant son.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BM2MyNjYxNmUtYTAwNi00MTYxLWJmNWYtYzZlODY3ZTk3OTFlXkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_FMjpg_UX1000_.jpg',
    },
    {
      id: 3,
      title: 'The Dark Knight',
      year: 2008,
      genre: ['Action', 'Crime', 'Drama'],
      rating: 9,
      director: 'Christopher Nolan',
      writer: 'Jonathan Nolan, Christopher Nolan',
      cast: ['Christian Bale', 'Heath Ledger'],
      summary:
        'When the menace known as The Joker emerges, Batman must face one of the greatest psychological tests of his ability to fight injustice.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BMTk4ODQzNDY3Ml5BMl5BanBnXkFtZTcwODA0NTM4Nw@@._V1_FMjpg_UX1000_.jpg',
    },
    {
      id: 4,
      title: 'Pulp Fiction',
      year: 1994,
      genre: ['Crime', 'Drama'],
      rating: 8,
      director: 'Quentin Tarantino',
      writer: 'Quentin Tarantino',
      cast: ['John Travolta', 'Samuel L. Jackson'],
      summary:
        'The lives of two mob hitmen, a boxer, a gangster and his wife, and a pair of diner bandits intertwine in four tales of violence and redemption.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BNGNhMDIzZTUtNTBlZi00MTRlLWFjM2ItYzViMjE3YzI5MjljXkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg',
    },
    {
      id: 5,
      title: 'Return of the King',
      year: 2003,
      genre: ['Adventure', 'Drama', 'Fantasy'],
      rating: 8,
      director: 'Peter Jackson',
      writer: 'J.R.R. Tolkien, Fran Walsh, Philippa Boyens, Peter Jackson',
      cast: ['Elijah Wood', 'Viggo Mortensen'],
      summary:
        "Gandalf and Aragorn lead the World of Men against Sauron's army to draw his gaze from Frodo and Sam as they approach Mount Doom with the One Ring.",
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BNzA5ZDNlZWMtM2NhNS00NDJjLTk4NDItYTRmY2EwMWZlMTY3XkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg',
    },
    {
      id: 6,
      title: 'Forrest Gump',
      year: 1994,
      genre: ['Drama', 'Romance'],
      rating: 8,
      director: 'Robert Zemeckis',
      writer: 'Winston Groom, Eric Roth',
      cast: ['Tom Hanks', 'Robin Wright'],
      summary:
        'The presidencies of Kennedy and Johnson, the events of Vietnam, Watergate, and other history unfold through the perspective of an Alabama man with an IQ of 75.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BNWIwODRlZTUtY2U3ZS00Yzg1LWJhNzYtMmZiYmEyNmU1NjMzXkEyXkFqcGdeQXVyMTQxNzMzNDI@._V1_.jpg',
    },
    {
      id: 7,
      title: 'Inception',
      year: 2010,
      genre: ['Action', 'Adventure', 'Sci-Fi'],
      rating: 8,
      director: 'Christopher Nolan',
      writer: 'Christopher Nolan',
      cast: ['Leonardo DiCaprio', 'Joseph Gordon-Levitt'],
      summary:
        'A thief who steals corporate secrets through the use of dream-sharing technology is given the inverse task of planting an idea into the mind of a C.E.O.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BMjAxMzY3NjcxNF5BMl5BanBnXkFtZTcwNTI5OTM0Mw@@._V1_.jpg',
    },
    {
      id: 8,
      title: 'Fight Club',
      year: 1999,
      genre: ['Drama'],
      rating: 8,
      director: 'David Fincher',
      writer: 'Chuck Palahniuk, Jim Uhls',
      cast: ['Brad Pitt', 'Edward Norton'],
      summary:
        'An insomniac office worker and a devil-may-care soapmaker form an underground fight club that evolves into something much, much more.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BNDIzNDU0YzEtYzE5Ni00ZjlkLTk5ZjgtNjM3NWE4YzA3Nzk3XkEyXkFqcGdeQXVyMjUzOTY1NTc@._V1_.jpg',
    },
    {
      id: 9,
      title: 'The Matrix',
      year: 1999,
      genre: ['Action', 'Sci-Fi'],
      rating: 8,
      director: 'Lana Wachowski, Lilly Wachowski',
      writer: 'Lana Wachowski, Lilly Wachowski',
      cast: ['Keanu Reeves', 'Laurence Fishburne'],
      summary:
        'A computer hacker learns from mysterious rebels about the true nature of his reality and his role in the war against its controllers.',
      imageURL: 'https://www.whatisthematrix.com/assets/images/fullbanner.jpg',
    },
    {
      id: 10,
      title: 'Goodfellas',
      year: 1991,
      genre: ['Biography', 'Crime', 'Drama'],
      rating: 8,
      director: 'Martin Scorsese',
      writer: 'Nicholas Pileggi, Martin Scorsese',
      cast: ['Robert De Niro', 'Ray Liotta'],
      summary:
        'The story of Henry Hill and his life in the mob, covering his relationship with his wife Karen Hill and his mob partners Jimmy Conway and Tommy DeVito.',
      imageURL:
        'https://m.media-amazon.com/images/M/MV5BY2NkZjEzMDgtN2RjYy00YzM1LWI4ZmQtMjIwYjFjNmI3ZGEwXkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg',
    },
    {
      id: 11,
      title: 'Fellowship of the Ring',
      year: 2001,
      genre: ['Adventure', 'Drama', 'Fantasy'],
      rating: 8,
      director: 'Peter Jackson',
      writer: 'J.R.R. Tolkien, Fran Walsh, Philippa Boyens, Peter Jackson',
      cast: ['Elijah Wood', 'Ian McKellen'],
      summary:
        'A young hobbit, Frodo, who has found the One Ring, begins his journey to Mount Doom, where he can destroy it.',
      imageURL:
        'https://images.moviesanywhere.com/198e228b071c60f5ad57e5f62fe60029/ff22cad6-2218-414d-b853-3f95d76905c7.jpg',
    },
  ]

  // export const sortBySearchparams = (allMovies, searchTerm) => {

  //   if (searchTerm?.length > 0) {
  //     const searchData = [];
  //     allMovies.forEach((m) => {
  //       searchTerm.forEach((i) => {
  //         if (((m.title.toLowerCase().includes(i.toLowerCase()) || m.director.toLowerCase().includes(i.toLowerCase())) && !searchData.includes(m))) {
  //           searchData.push(m);
  //         }
  //         else {
  //           m.cast.forEach((c) => {
  //             if (c.toLowerCase().includes(i.toLowerCase()) && !searchData.includes(m)) {
  //               searchData.push(m);
  //             }
  //           })
  //         }
  //       })
  //     })
  //     return searchData;
  //   }
  //   return movies;
  // }

  // const getFilteredMoviesByGenre = (movies,genre)=>{
  //   return movies.filter((m)=>{
  //    return m.genre.map((i)=>i.toLowerCase().includes(genre.toLowerCase()))
  //   })
  // }

  // const getfilteredMoviesByYear =(movies,year)=>{
  //  return  movies.filter((m)=> m.year.toString()===year.toString())
  // }
  // const getFilteredProductsByRating = (movies,rating)=>{
  //   return movies.filter((m)=> m.rating.toString()===rating.toString())
  // }


  // export const getTransformedMovies = ({
  //   allMovies,
  //   searchTerm,
  //   selectedGenre="",
  //   selectedYear ="",
  //   selectedRating =""
  // }) => {
  //   const getsortBySearchparams = sortBySearchparams(
  //     allMovies,
  //     searchTerm
  //   );
  //   console.log(getsortBySearchparams);
  //   // const filteredMoviesByGenre = getFilteredMoviesByGenre(
  //   //   getsortBySearchparams,
  //   //   selectedGenre
  //   // );
  //   // const filteredMoviesByYear = getfilteredMoviesByYear(
  //   //   filteredMoviesByGenre,
  //   //   selectedYear
  //   // );
  //   // const sortedProductList = getFilteredProductsByRating(
  //   //   filteredMoviesByYear,
  //   //   selectedRating
  //   // );
  
  //     return getsortBySearchparams;
    
  // };